...
class JsExample {
    ...
    public static var d = Browser.document;
	
	public static var input:Input = new Input(cast d.getElementById("input"));
	public static var ast:TextAreaElement = cast d.getElementById("ast");
	public static var output:TextAreaElement = cast d.getElementById("output");

    public function new() {
        trace(input, ast, output);
        input.area.addEventListener("keyup", function(_) {
			try {
				ast.value = little.tools.PrettyPrinter.printParserAst(little.parser.Parser.parse(little.lexer.Lexer.lex(input.area.value)));
			} catch (e) {}

			try {
				Little.run(input.area.value, true);
				output.value = Runtime.stdout;
			} catch (e) {}
		});

        input.area.onkeydown = function (e) {
            if (e.key == 'Tab') {
                e.preventDefault();
                var start = input.area.selectionStart;
                var end = input.area.selectionEnd;

                // set textarea value to: text before caret + tab + text after caret
                input.area.value = input.area.value.substring(0, start) + "\t" + input.area.value.substring(end);

                // put caret at right position again
                input.area.selectionStart = input.area.selectionEnd = start + 1;
            }
        }
        ...
        function update() {
            ...
            for (row in keywordTable.rows) {
                ...
                p.innerText = getCodeExample(input.area.id);
                p.onchange();
            }
        }
        ...
        for (keyword in ["VARIABLE_DECLARATION", "FUNCTION_DECLARATION", "NULL_VALUE", "RUN_CODE_FUNCTION_NAME", "TYPE_DECL_OR_CAST", "TYPE_FLOAT", "TYPE_BOOLEAN", "TYPE_STRING", "TYPE_MODULE", "FUNCTION_RETURN", "READ_FUNCTION_NAME", "TYPE_DYNAMIC", "PROPERTY_ACCESS_SIGN", "TRUE_VALUE", "TYPE_INT", "FALSE_VALUE", "RAISE_ERROR_FUNCTION_NAME", "TYPE_SIGN", "PRINT_FUNCTION_NAME"]) {
            ...
            var inputElement = d.createInputElement();
            inputElement.id = keyword;
            inputElement.placeholder = "single word, e.g. " + Reflect.field(Keywords.defaultKeywordSet, keyword);
            inputElement.onchange = () -> {
                Reflect.setField(Keywords, keyword, inputElement.value != null ? (inputElement.value != "" ? inputElement.value : Reflect.field(Keywords.defaultKeywordSet, keyword)) : Reflect.field(Keywords.defaultKeywordSet, keyword));
                update();
            }
